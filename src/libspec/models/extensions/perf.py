...
class MemoryLayoutSpec(ExtensionModel):
    size_bytes: ByteSize | None = Field(default=None, description='Instance size in bytes')
    alignment: PositiveInt | None = Field(default=None, description='Memory alignment requirement')
    cache_friendly: bool | None = Field(None, description='Whether layout is cache-optimized')
...
class ScalingSpec(ExtensionModel):
    ...
    max_concurrent: PositiveInt | None = Field(default=None, description='Maximum concurrent operations')
    sharding_supported: bool | None = Field(None, description='Whether sharding is supported')
